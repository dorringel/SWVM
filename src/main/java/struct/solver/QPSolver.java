...
public class QPSolver {
	...
	private final QPParameters qp_params;
	...
	public double[] hildreth_sequential_fast(QPConstraints constraints) {
		...
		while (cont) {
			...
			logger.info("max_kkt_i = " + max_kkt_i + " max_kkt = " + max_kkt + " iter = " + iter
	...
	public QPOutput hildreth_sequential_alg_b00_full(QPConstraints cns) {
		...
		logger.info(" min_nrm1 = " + min_nrm1 + " min_nrm2 = " + min_nrm2);
		...
		while (cont) {
			...
			for (int ind = 0; ind < K; ind++) {
				...
				if (kkt == Double.POSITIVE_INFINITY) {
					logger.warning(" ERROR: kkt= " + kkt + " ind= " + ind + " timer[ind]= " + timer[ind]
							+ " alpha[ind]= " + alpha[ind] + " b[ind]= " + cns.b[ind] + " inner1= " + inner1
							+ " inner2= " + inner2);
				...
				if (kkt > qp_params.zero) {
					...
					if (Math.abs(diff_alpha) == Double.POSITIVE_INFINITY)
						logger.warning(
								"ERROR: diff_alpha= " + diff_alpha + " ind= " + ind + " alpha[ind]=" + alpha[ind]);
			...
			for (i = 0; i < cns.dim2; i++) {
				...
				if (kkt_f == Double.POSITIVE_INFINITY) {
					logger.warning(" ERROR: kkt_f= " + kkt_f + " i= " + i);
				}
				if (kkt_f > max_kkt_f) {
					max_kkt_f = kkt_f;
					max_kkt_f_i = i;
			...
			logger.info(
					"max_kkt_i = " + max_kkt_i + " max_kkt = " + max_kkt + " iter = " + iter + " no_skip_constraints = "
							+ no_skip_constraints + " max_kkt_f_i = " + max_kkt_f_i + " max_kkt_f = " + max_kkt_f
							+ " iter = " + iter + " nu= " + nu + " invnu= " + invnu + " eps = " + eps);
	...
	public double[] hildreth_sequential_alg_b00_blas(QPConstraints cns) {
		...
		while (cont) {
			...
			logger.info("max_kkt_i = " + max_kkt_i + " max_kkt = " + max_kkt + " iter = " + iter
					+ " no_skip_constraints = " + no_skip_constraints + " max_kkt_f_i = " + max_kkt_f_i
					+ " max_kkt_f = " + max_kkt_f + " iter = " + iter + " nu= " + nu + " eps = " + eps);
	...
	public static double[] hildreth(SLFeatureVector[] a, double[] b) {
		...
		while (max_kkt >= EPS && iter < MAX_ITER) {
			...
			if (!is_computed[max_kkt_i]) {
				for (i = 0; i < K; i++) {
					A[i][max_kkt_i] = SLFeatureVector.dotProduct(a[i], a[max_kkt_i]); // for
																						// version
																						// 1
